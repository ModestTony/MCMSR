...
from mmsr.utils import (MessageLogger, get_root_logger, init_tb_logger,make_exp_dirs, set_random_seed)
...
def main():
    ...
    logger = get_root_logger(logger_name='base', log_level=logging.INFO, log_file=log_file)
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            if opt['dist']:
                train_sampler = DistIterSampler(train_set, world_size, rank,dataset_enlarge_ratio)
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            # validation
            if opt['datasets']['val'] and current_iter % opt['val']['val_freq'] == 0:
