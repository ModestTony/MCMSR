...
def feature_match_index(feat_input,
    ...
    for idx in range(0, n_patches, batch_size):
        ...
        corr = F.conv2d(
            ...
            stride=input_stride)
        topk = 2
        max_val_tmp, max_idx_tmp = torch.topk(corr.squeeze(0),topk,dim=0,largest=True,sorted=True)
    ...
    if norm_input:
        ...
        # norm = norm.unsqueeze(0).expand(topk, int((h - patch_size) / input_stride + 1),int((w - patch_size) / input_stride + 1))
