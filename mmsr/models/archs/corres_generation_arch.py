...
class CorrespondenceGenerationArch(nn.Module):
    ...
    def forward(self, dense_features, img_ref_hr):
        ...
        for ind in range(img_ref_hr.size(0)):
            ...
            # pdb.set_trace()
            _max_idx, _max_val = feature_match_index(feat_in, feat_ref, patch_size=self.patch_size,
                                                     input_stride=self.stride, ref_stride=self.stride, is_norm=True,
                                                     norm_input=True)
            # pdb.set_trace()
            max_val.append(_max_val)
            # _max_idx=[38,38],_max_val=[38,38]
            # pdb.set_trace()
            topk, _, _ = _max_idx.size()
            # topk = 1
            for i in range(topk):
                # offset map for relu3_1
                offset_relu3 = self.index_to_flow(_max_idx[i])
                # shift offset relu3
        ...
        max_val = torch.nn.functional.pad(max_val, (1, 1, 1, 1, 0, 0))
