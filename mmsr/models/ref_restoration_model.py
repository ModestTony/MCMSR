...
class RefRestorationModel(SRModel):
    ...
    def optimize_parameters(self, step):
        ...
        # pdb.set_trace()
        self.pre_offset, self.img_ref_feat, self.simi = self.net_map(self.features, self.img_ref)
        # t2 = time.time()
        # pdb.set_trace()
        self.output = self.net_g(self.img_in_lq, self.pre_offset,self.img_ref_feat,self.simi)
        # t3 = time.time()
        # pdb.set_trace()
    ...
    def test(self):
        ...
        with torch.no_grad():
            match_in = [self.match_img_in]
            ref = [self.img_ref]
            lr = [self.img_in_lq]
            for tf in 'v', 'h', 't':
                match_in.extend([_transform(t, tf) for t in match_in])
            for tf in 'v', 'h', 't':
                ref.extend([_transform(t, tf) for t in ref])
            for tf in 'v', 'h', 't':
                lr.extend([_transform(t, tf) for t in lr])

            list_y = []
            for i in range(len(match_in)):
                #pdb.set_trace()
                self.features = self.net_extractor(match_in[i], ref[i])

                self.pre_offset, self.img_ref_feat, self.simi = self.net_map(self.features, ref[i])
                #pdb.set_trace()
                y = self.net_g(lr[i], self.pre_offset, self.img_ref_feat, self.simi)
            ...
            y = [torch.cat(_y, dim=0).mean(dim=0, keepdim=True) for _y in list_y]  # average
            if len(y) == 1: y = y[0]
            self.output = y
        self.net_g.train()
